...
export default class Diff extends React.Component<DiffProps, DiffState> {
  ...
  componentWillMount() {
    if (
      !this.props.diffs &&
      !this.state.diffs &&
      this.props.javascriptCode &&
      this.props.typescriptCode
    ) {
    ...
    return (
      <div className="Diff">
        <div className="pane">
          ...
          <div className="codeArea">{this.buildCode(this.props.diffs || this.state.diffs)}</div>
        </div>
      </div>
    );
  }
  ...
  buildMargin = () => {
    ...
    const lineNumbers = _.range(numLines).map((n: number) => {
      return (
        <div className={`lineNumber line${n}`} key={n}>
          {n + 1}
        </div>
      );
    });

    return <div className="margin">{lineNumbers}</div>;
  };

  buildCode = (diffs: Change[] | null) => {
    if (!diffs) {
    ...
    return diffs.map((d, i) => {
      ...
      if (d.added) {
        return (
          <span className="added bubble" key={i}>
            {value}
          </span>
        );
      } else if (d.removed) {
        return (
          <span className="removed bubble" key={i}>
            {value}
          </span>
        );
      } else {
